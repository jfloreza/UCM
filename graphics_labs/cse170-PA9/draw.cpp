...
Vec eval_crspline ( float t, std::vector<Vec>& pnts )
{
	Vec P, I;
	for (int i = 1; i < pnts.size() - 1; i++)
	{
		I.x = (pnts[i-1].x - pnts[i+1].x) * 0.5;
		I.y = (pnts[i-1].y - pnts[i+1].y) * 0.5;

		P.x += pnts[i].x + float(I.x / 3.0);
		P.y += pnts[i].y + float(I.y / 3.0);
	}

	return P;

}
Vec eval_bospline ( float t, std::vector<Vec>& pnts )
{
	Vec B;
	return B;
